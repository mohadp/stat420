...
- Model performance: adjusted r-squared, AIC, BIC, leave one out cross-validation, and rmse on test data.
...
We can see that model assumptions are clearly violated. When looking at variables, it would make sense to only use investment_rounds or invested_companies, because there's collinearity between both.

```{r}
startups_tr$aquisition_price_amount[abs(rstandard(additive_full)) > 2]
range(startups_tr$aquisition_price_amount)
mean(startups_tr$aquisition_price_amount)
```
