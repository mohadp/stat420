...
We remove the invested_companies and state_code from the predictors.
...
#Diagnostics and performance of the model
diagnostics_performance(additive_full_updated, predict_function = function (x) {exp(x)})
...
In the plots, we can see that normality and constant variance are still suspect. We also see confirm that normality suspect due to the low shapiro-wilk test p-value, and that the constant variance is suspect due to low bp test p-value. The R squared and adjusted R squared remain somewhat similar, as well as the RMSE leave one out cross validation. We now have less variables with a potentially large collinearity. We are ok to keep them for now: for completeness sake when performing predictions, we want to be able to still provide the business category code, the city, and the country.

Now we will perform a backwards AIC search to optimize for AIC.
```{r}
#Finding the model
...
The found model still violates normality and constant variance assumptions, as would normally be expected since this is derived from a model that also violated them. However, we are able to see less predictors with a variance inflation factor higher than 5. Now we proceed to compare both models with an anova test.

```{r}
anova(model_additive_selected, additive_full_updated)
```

We can see that the larger model doesn't offer a statistically significant difference in explaining the relationship of predictors and response. For thatt reason, we prefer the model found through the backwards BIC method, `model_additive_selected`.
...
full_model = lm(log(aquisition_price_amount) ~ . , data = startups_tr)
startups_te$aquisition_price_amount[1]
exp(predict(full_model, newdata = startups_te)[1])


predict_function = function (x) {exp(x)}
(model_RMSE = sqrt(mean(sum(exp(predict(full_model, newdata = startups_te)) - startups_te$aquisition_price_amount)^2)))

diagnostics(full_model, predict_function = function (x) {exp(x)})
startups_te$aquisition_price_amount
...
boxcox(additive_model, plotit = TRUE, lambda = seq(0.01, 0.14, by = 0.001))

additive_model_mod = lm((aquisition_price_amount)^0.11~(city + category_code + investment_rounds + invested_companies + funding_rounds + log(funding_total_usd+1) + milestones + log(relationships+1) + istop25), data = startups_tr)
...
overall_interaction_model_mod = lm(aquisition_price_amount^0.130~(category_code + investment_rounds + invested_companies + funding_rounds + log(funding_total_usd+1) + milestones + log(relationships+1) + istop25)^3 + city, data = startups_tr)

startups_te$aquisition_price_amount[1]
predict(overall_interaction_model_mod, newdata = startups_te)[1]
