...
The default RMSE for box-cox transformation models by default returned `Nan` for some test observations; these values caused negative prediction values. Upon converting the box-cox value from `model_transformation_boxcox` into dollars (`aquisition_price_amount`), using the the box-cox inverted function, we end up doing a power to the 1/2n, as in the example below, resulting in `NaN`s; in this case this only affects 13 observations of all test data.
