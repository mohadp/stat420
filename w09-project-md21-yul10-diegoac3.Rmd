...
#Diagnostics and performance of the model
diagnostics_performance(model_additive_selected_AIC)

#Variance inflation factors
faraway::vif(model_additive_selected_AIC)[faraway::vif(model_additive_selected_AIC)>5]

#Outliers
c(
  num_outliers = length(rstandard(model_additive_selected_AIC)[abs(rstandard(model_additive_selected_AIC)) > 2]),
  pct_outliers = length(rstandard(model_additive_selected_AIC)[abs(rstandard(model_additive_selected_AIC)) > 2]) / length(rstandard(model_additive_selected_AIC))
)

#Influential points
c(
  num_influential_points = sum(cooks.distance(model_additive_selected_AIC) > 4 / length(cooks.distance(model_additive_selected_AIC))),
  pct_influential_points = sum(cooks.distance(model_additive_selected_AIC) > 4 / length(cooks.distance(model_additive_selected_AIC))) / length(cooks.distance(model_additive_selected_AIC))
)
...
We can see that the larger model doesn't offer a statistically significant difference in explaining the relationship of predictors and response. For that reason, we prefer the model found through the backwards BIC method, `model_additive_selected_AIC`. This model still violates normality and constant variance assumptions, and these will not likely be corrected or at least further improved until we look at more complex interaction models or we apply transformations to predictor or response variables (done below).
...
#Training the model
model_transformation_boxcox = lm(aquisition_price_amount^0.12 ~ (. -investment_rounds -state_code)^2 + log(funding_total_usd+1) +log(relationships+1) , data = startups_tr)

#Diagnostics and performance of the model
diagnostics_performance(model_transformation_boxcox)
...
```{r warning=FALSE}
#Finding the model
model_transformation_boxcox_start = lm(aquisition_price_amount ^ 0.12 ~ 1, data = startups_tr)
model_transformation_boxcox_selected = step(
  model_transformation_boxcox_start,
  scope =
    aquisition_price_amount ^ 0.12 ~ category_code * funding_rounds + category_code * milestones +
    category_code * aquisition_price_amount + category_code * top50pct + category_code *
    istop26_50 + category_code * country_code + category_code * city + category_code *
    invested_companies + category_code * funding_total_usd + category_code *
    relationships + category_code * top25pct + category_code * istop25 + category_code *
...
#Diagnostics and performance of the model
diagnostics_performance(model_transformation_boxcox_selected)

#Number of predictors
length(coef(model_transformation_boxcox_selected))
...
#Diagnostics and performance of the model
diagnostics_performance(model_transformation_log)
```
...
In summary, the three preferred models are: `model_additive_selected_AIC` (additive), `interaction_model` (interaction), `model_transformation_boxcox` (transformation).
